...
def myPrint_route_HTML(nodes, is_proven, logger, route_num, smiles, route_id, json_weights, save_tree, expansion_num, cum_prob_mod, chem_axon, substance,selection_constant, time_limit, csrf_token, fn, image_dir="/var/www/html/public/images"):
    ...
    message = "Reaction route search done."
...
#komai
    ...
    for node_index, node in enumerate(nodes):
        ...
        for reactant_index, mol in enumerate(node.state.mols):
            thisSmiles=Chem.MolToSmiles(mol)
            if thisSmiles not in next_depth_smiles:
                reactant_id = reactant_index + 1
                route_summary += f"""<div class="molecule"><p>{thisSmiles}</p></div>"""
                small.append(thisSmiles)
                num_reactants += 1
        ...
        if node_index + 1 < len(nodes): 
            ...
            for i, mol in enumerate(nodes[node_index + 1].state.mols):
                thisSmiles=Chem.MolToSmiles(mol)
                if thisSmiles not in current_depth_smiles:
                    product_id = i + 1
                    small.append(thisSmiles)
                    route_summary += f"""<div class="molecule"><p>{thisSmiles}</p></div>"""
